	...
	function build() {
		...
		svg.Property = function(name, value) {
			...
			this.Length = {
				...
				EM: function(viewPort) {
					...
					var fontSize = new svg.Property('fontSize', svg.Font.Parse(svg.context.font).fontSize);
		...
		svg.Font = new (function() {
			...
			this.CreateFont = function(fontStyle, fontVariant, fontWeight, fontSize, fontFamily, inherit) { 
				var f = inherit != null ? this.Parse(inherit) : this.CreateFont('', '', '', '', '', svg.context.font);
				return { 
		...
		svg.Transform = function(v) {	
			...
			this.Type.translate = function(s) {
				...
				this.apply = function(ctx) {
					context.translate(this.p.x || 0.0, this.p.y || 0.0);
				}
			...
			this.Type.rotate = function(s) {
				...
				this.apply = function(ctx) {
					...
					context.rotate(this.angle.Angle.toRadians());
					context.translate(-this.cx, -this.cy);
				}
				this.applyToPoint = function(p) {
			...
			this.Type.scale = function(s) {
				this.p = svg.CreatePoint(s);
				this.apply = function(ctx) {
					context.scale(this.p.x || 1.0, this.p.y || this.p.x || 1.0);
				}
				this.applyToPoint = function(p) {
		...
		svg.AspectRatio = function(ctx, aspectRatio, width, desiredWidth, height, desiredHeight, minX, minY, refX, refY) {
			...
			if (refX.hasValue() && refY.hasValue()) {				
				context.translate(-scaleMin * refX.Length.toPixels('x'), -scaleMin * refY.Length.toPixels('y'));
			} 
			else {					
				// align
				if (align.match(/^xMid/) && ((meetOrSlice == 'meet' && scaleMin == scaleY) || (meetOrSlice == 'slice' && scaleMax == scaleY))) context.translate(width / 2.0 - desiredWidth / 2.0, 0); 
				if (align.match(/YMid$/) && ((meetOrSlice == 'meet' && scaleMin == scaleX) || (meetOrSlice == 'slice' && scaleMax == scaleX))) context.translate(0, height / 2.0 - desiredHeight / 2.0); 
				if (align.match(/^xMax/) && ((meetOrSlice == 'meet' && scaleMin == scaleY) || (meetOrSlice == 'slice' && scaleMax == scaleY))) context.translate(width - desiredWidth, 0); 
				if (align.match(/YMax$/) && ((meetOrSlice == 'meet' && scaleMin == scaleX) || (meetOrSlice == 'slice' && scaleMax == scaleX))) context.translate(0, height - desiredHeight); 
			}
			
			// scale
			if (align == 'none') context.scale(scaleX, scaleY);
			else if (meetOrSlice == 'meet') context.scale(scaleMin, scaleMin); 
			else if (meetOrSlice == 'slice') context.scale(scaleMax, scaleMax); 	
			
			// translate
			context.translate(minX == null ? 0 : -minX, minY == null ? 0 : -minY);			
		}
		...
		svg.Element.RenderedElementBase = function(node) {
			...
			this.setContext = function(ctx) {
				...
				if (this.style('fill').Definition.isUrl()) {
					...
					if (fs != null) context.fillStyle = fs;
				}
				else if (this.style('fill').hasValue()) {
					var fillStyle = this.style('fill');
					if (this.style('fill-opacity').hasValue()) fillStyle = fillStyle.Color.addOpacity(this.style('fill-opacity').value);
					context.fillStyle = (fillStyle.value == 'none' ? 'rgba(0,0,0,0)' : fillStyle.value);
				}
									
				// stroke
				if (this.style('stroke').Definition.isUrl()) {
					var fs = this.style('stroke').Definition.getFillStyle(this);
					if (fs != null) context.strokeStyle = fs;
				}
				else if (this.style('stroke').hasValue()) {
					var strokeStyle = this.style('stroke');
					if (this.style('stroke-opacity').hasValue()) strokeStyle = strokeStyle.Color.addOpacity(this.style('stroke-opacity').value);
					context.strokeStyle = (strokeStyle.value == 'none' ? 'rgba(0,0,0,0)' : strokeStyle.value);
				}
				if (this.style('stroke-width').hasValue()) context.lineWidth = this.style('stroke-width').Length.toPixels();
				if (this.style('stroke-linecap').hasValue()) context.lineCap = this.style('stroke-linecap').value;
				if (this.style('stroke-linejoin').hasValue()) context.lineJoin = this.style('stroke-linejoin').value;
				if (this.style('stroke-miterlimit').hasValue()) context.miterLimit = this.style('stroke-miterlimit').value;

				// font
				if (typeof(context.font) != 'undefined') {
					context.font = svg.Font.CreateFont( 
				...
				if (this.style('opacity').hasValue()) {
					context.globalAlpha = this.style('opacity').numValue();
				}
			}		
		}
		...
		svg.Element.svg = function(node) {
			...
			this.setContext = function(ctx) {
				...
				if (this.attribute('x').hasValue() && this.attribute('y').hasValue()) {
					context.translate(this.attribute('x').Length.toPixels('x'), this.attribute('y').Length.toPixels('y'));
				}
		...
		svg.Element.polyline = function(node) {
			...
			this.path = function(ctx) {
				...
				for (var i=1; i<this.points.length; i++) {
					...
					if (ctx != null) context.lineTo(this.points[i].x, this.points[i].y);
				}
				return bb;
			}
			...
			this.path = function(ctx) {
				...
				while (!pp.isEnd()) {
					...
					case 'M':
						...
						pp.addMarker(p);
						bb.addPoint(p.x, p.y);
						if (ctx != null) context.moveTo(p.x, p.y);
						pp.start = pp.current;
						while (!pp.isCommandOrEnd()) {
							var p = pp.getAsCurrentPoint();
							pp.addMarker(p, pp.start);
							bb.addPoint(p.x, p.y);
							if (ctx != null) context.lineTo(p.x, p.y);
						}
						break;
					case 'L':
						while (!pp.isCommandOrEnd()) {
					...
					case 'H':
						while (!pp.isCommandOrEnd()) {
							...
							if (ctx != null) context.lineTo(pp.current.x, pp.current.y);
						}
						break;
					case 'V':
						while (!pp.isCommandOrEnd()) {
					...
					case 'C':
						while (!pp.isCommandOrEnd()) {
							...
							if (ctx != null) context.bezierCurveTo(p1.x, p1.y, cntrl.x, cntrl.y, cp.x, cp.y);
						}
						break;
					case 'S':
						while (!pp.isCommandOrEnd()) {
					...
					case 'Q':
						while (!pp.isCommandOrEnd()) {
							...
							if (ctx != null) context.quadraticCurveTo(cntrl.x, cntrl.y, cp.x, cp.y);
						}
						break;
					case 'T':
						while (!pp.isCommandOrEnd()) {
		...
		svg.Element.marker = function(node) {
			...
			this.render = function(ctx, point, angle) {
				...
				if (this.attribute('orient').valueOrDefault('auto') == 'auto') context.rotate(angle);
				if (this.attribute('markerUnits').valueOrDefault('strokeWidth') == 'strokeWidth') context.scale(context.lineWidth, context.lineWidth);
				context.save();
				...
				context.restore();
				if (this.attribute('markerUnits').valueOrDefault('strokeWidth') == 'strokeWidth') context.scale(1/context.lineWidth, 1/context.lineWidth);
				if (this.attribute('orient').valueOrDefault('auto') == 'auto') context.rotate(-angle);
				context.translate(-point.x, -point.y);
			}
		}
		...
		svg.Element.radialGradient = function(node) {
			...
			this.getGradient = function(ctx, element) {
				...
				return context.createRadialGradient(fx, fy, 0, cx, cy, r);
			}
		}
		...
		svg.Element.text = function(node) {
			...
			this.setContext = function(ctx) {
				this.baseSetContext(ctx);
				if (this.style('dominant-baseline').hasValue()) context.textBaseline = this.style('dominant-baseline').value;
				if (this.style('alignment-baseline').hasValue()) context.textBaseline = this.style('alignment-baseline').value;
			}
		...
		svg.Element.TextElementBase = function(node) {
			...
			this.renderChildren = function(ctx) {
				...
				if (customFont != null) {
					var fontSize = this.parent.style('font-size').numValueOrDefault(svg.Font.Parse(svg.context.font).fontSize);
					var fontStyle = this.parent.style('font-style').valueOrDefault(svg.Font.Parse(svg.context.font).fontStyle);
					...
					for (var i=0; i<text.length; i++) {
						...
						context.lineWidth = context.lineWidth * customFont.fontFace.unitsPerEm / fontSize;
						if (fontStyle == 'italic') context.transform(1, 0, .4, 1, 0, 0);
						glyph.render(ctx);
						if (fontStyle == 'italic') context.transform(1, 0, -.4, 1, 0, 0);
				...
				if (context.strokeStyle != '') context.strokeText(svg.compressSpaces(this.getText()), this.x, this.y);
				if (context.fillStyle != '') context.fillText(svg.compressSpaces(this.getText()), this.x, this.y);
			}
			...
			this.measureText = function(ctx) {
				...
				if (!context.measureText) return textToMeasure.length * 10;
				
				context.save();
				this.setContext(ctx);
				var width = context.measureText(textToMeasure).width;
				context.restore();
				return width;
			}
		}
		...
		svg.Element.a = function(node) {
			...
			this.onmousemove = function() {
				svg.context.canvas.style.cursor = 'pointer';
			}
		}
		...
		svg.Element.use = function(node) {
			...
			this.setContext = function(ctx) {
				this.baseSetContext(ctx);
				if (this.attribute('x').hasValue()) context.translate(this.attribute('x').Length.toPixels('x'), 0);
				if (this.attribute('y').hasValue()) context.translate(0, this.attribute('y').Length.toPixels('y'));
			}
		...
		svg.Element.mask = function(node) {
			...
			this.apply = function(ctx, element) {
				...
				element.attribute('mask').value = '';
					...
					context.fillStyle = tempCtx.createPattern(c, 'no-repeat');
					context.fillRect(0, 0, x + width, y + height);
		...
		svg.Element.filter = function(node) {
			...
			this.apply = function(ctx, element) {
				...
				// render on me
				context.drawImage(c, 0, 0, width + 2*px, height + 2*py, x - px, y - py, width + 2*px, height + 2*py);
		...
		svg.loadXmlDoc = function(ctx, dom) {
			...
			var draw = function() {
				svg.ViewPort.Clear();
				if (context.canvas.parentNode) svg.ViewPort.SetCurrent(context.canvas.parentNode.clientWidth, context.canvas.parentNode.clientHeight);
			
				if (svg.opts['ignoreDimensions'] != true) {
					// set canvas size
					if (e.style('width').hasValue()) {
						context.canvas.width = e.style('width').Length.toPixels('x');
						context.canvas.style.width = context.canvas.width + 'px';
					}
					if (e.style('height').hasValue()) {
						context.canvas.height = e.style('height').Length.toPixels('y');
						context.canvas.style.height = context.canvas.height + 'px';
					}
				}
				var cWidth = context.canvas.clientWidth || context.canvas.width;
				var cHeight = context.canvas.clientHeight || context.canvas.height;
		...
		svg.Mouse = new (function() {
			...
			this.checkPath = function(element, ctx) {
				for (var i=0; i<this.events.length; i++) {
					var e = this.events[i];
					if (context.isPointInPath && context.isPointInPath(e.x, e.y)) this.eventElements[i] = element;
				}
			}
			...
			this.runEvents = function() {
				svg.context.canvas.style.cursor = '';
